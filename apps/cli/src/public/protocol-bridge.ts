...
function diagnosticsFor(
  ...
  const category =
    result.operationalStatus === "invalid"
      ? "invalid"
...
function verifyProtocolResult(
  ...
  if (
    result.operationalStatus !== "completed" &&
    result.operationalStatus !== "blocked"
  ) {
    return null;
  }
  ...
  const promises = Array.isArray(enriched.promises)
    ? enriched.promises
    : [];
  ...
  const workspace =
    result.workspace.binding === undefined ||
    result.workspace.modelRevision === undefined
      ? undefined
      : {
          rootBinding: result.workspace.binding,
  ...
  return {
    ...
    ...(workspace === undefined
      ...
      : {
          workspace:
            workspace as unknown as NonNullable<VerifyResult["workspace"]>,
        }),
    reasonCodes: result.reasonCodes,
    applicationModel:
      result.applicationModel as unknown as VerifyResult["applicationModel"],
...
export function toProtocolEnvelope(
  ...
  return {
    ...
    command: "verify",
    invocationId:
      result.invocationId as AnyCommandEnvelope["invocationId"],
    engine: {
    ...
    result: verifyProtocolResult(result),
...
export function toJsonlEventRecord(
  ...
  return {
    ...
    event: {
      schemaVersion: 1,
      eventId:
        `${result.invocationId}:event:${event.sequence}` as JsonlEventRecord["event"]["eventId"],
      eventType: event.type,
      occurredAt,
      invocationId:
        result.invocationId as JsonlEventRecord["event"]["invocationId"],
      correlationId:
        result.invocationId as JsonlEventRecord["event"]["correlationId"],
      sequence: event.sequence,
      producer: {
        id:
          "@verify-internal/engine" as JsonlEventRecord["event"]["producer"]["id"],
        version: result.engineVersion,
        artifactDigest: ENGINE_ARTIFACT_DIGEST,
      },

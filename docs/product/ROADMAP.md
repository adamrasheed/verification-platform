...
| M6-T06 | **In progress** — Linux production is complete; macOS App Sandbox and Windows AppContainer development hosts pass all canaries; the protected signing/notarization pipeline and exact-pin manifests are complete; production certificates and signed artifacts pending | Implement signed native sandbox hosts | Linux namespaces+seccomp, macOS App Sandbox, Windows AppContainer canaries |
| M6-T07 | **Linux complete; macOS and Windows development complete; signed production artifact run pending** | Pass three synthetic providers with different auth, latency, and error behavior | Same plugins must pass again through each production native host |

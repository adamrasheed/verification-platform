...
| M6-T06 | **In progress** — Linux production is complete; macOS App Sandbox and Windows AppContainer development hosts pass filesystem/process/network/memory/CPU canaries and fail closed on unpinned identities; macOS and Windows release signing pending | Implement signed native sandbox hosts | Linux namespaces+seccomp, macOS App Sandbox, Windows AppContainer canaries |
| M6-T07 | **Linux complete; macOS and Windows development complete; production signing pending** | Pass three synthetic providers with different auth, latency, and error behavior | Same plugins must pass again through each production native host |

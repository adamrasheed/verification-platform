...
| M9-T05 | **In progress** — the provider-neutral queue port, digest-free reference envelope, idempotent consumer boundary, bounded retry/DLQ behavior, and CI conformance pass; the exact-queue AWS adapter, protected execution, and live secondary-sink scans remain | Implement SQS relay/worker and secondary-sink controls | Fenced at-least-once delivery, bounded retries, DLQ sanitization, deletion inventory, and canary scans pass |

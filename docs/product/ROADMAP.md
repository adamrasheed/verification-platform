...
| M9-T06 | **In progress** — the provider-neutral `/v1` adapter, checked OpenAPI 3.1 surface, bounded canonical JSON, exact action/resource authorization, short-lived Ed25519 workload identity, idempotency propagation, sanitized audit, and cross-tenant/IDOR matrix pass; PostgreSQL control state and protected AWS deployment remain | Deploy the tenant-authorized control API | Closed `/v1` schemas, exact actions/resources, idempotency, workload identity, and cross-tenant matrix pass |

...
RUN npm ci --ignore-scripts \
  ...
  && npm run build --workspace @verify-internal/auth \
  ...
  && npm run build --workspace @verify-internal/control-api \
...
COPY --from=build --chown=node:node /app/packages/cloud-client/fixtures ./packages/cloud-client/fixtures
...
COPY --from=build --chown=node:node /app/tooling/infra/run-live-control-api-conformance.mjs ./tooling/infra/run-live-control-api-conformance.mjs
COPY --chown=node:node tooling/infra/aws/metadata-cloud/us-west-2-bundle.pem /app/rds-ca-bundle.pem

USER node

CMD ["node", "tooling/infra/run-live-control-api-conformance.mjs"]
